...
export class AIService {
  ...
  static async analyzeInquiry(
    ...
    const department = RoutingService.determineDepartment(fullText, intent);
    const priority = RoutingService.predictPriority(sentiment, sentimentScore, fullText);

    // 4. Churn Risk Score Calculation (0.0 to 1.0)
    let churnScore = 0.15;
    if (sentimentScore < -0.6) churnScore += 0.55;
    else if (sentimentScore < -0.2) churnScore += 0.35;
    if (priority === 'critical') churnScore += 0.25;
